...
describe('Performance: Frame Rate During Interactions', () => {
  ...
  it('should maintain >55fps during dashboard resize', async () => {
    let frameCount = 0;
  ...
  it('should maintain >55fps during drag operations', async () => {
    let frameCount = 0;
  ...
  it('should maintain >55fps during chart updates', async () => {
    let frameCount = 0;
  ...
  it('should measure frame rate stability (min FPS should be close to average)', async () => {
    ...
    // Min FPS should be at least 80% of average FPS for stable performance
    const stabilityRatio = metrics.minFPS / metrics.averageFPS;
    expect(stabilityRatio).toBeGreaterThan(0.8);
